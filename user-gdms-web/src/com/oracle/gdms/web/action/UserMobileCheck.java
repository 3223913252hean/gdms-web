...
public class UserMobileCheck extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ResponseEntity ent = new ResponseEntity();                   //准备一个客户端返回的结果对象
		JSONObject json = new JSONObject();                          //JSON对象
		PrintWriter out =response.getWriter();   					//向客户端输出的流对象
		
		
		String mobile = request.getParameter("mobile");  //从请求中得到要检验的手机号码
		if (mobile==null) {
			ent.setCode(1002);
			...
			ent.setMessage(has ? "<span style='color:red'>该手机号码已被注册，请更改</apan>" : "<span style='color:green'>该手机号码可用</apan>");
		}
